...
std::vector<ConflictPtr> BeliefPlanValidityChecker::validatePlan(Plan p)
{
	...
	for (int k = 0; k < maxStates; k++) {
		std::unordered_map<std::string, std::pair<Eigen::Vector2d, Eigen::Matrix2d>> activeRobots = getActiveRobots_(p, k);
...
{
	std::unordered_map<std::string, std::pair<Eigen::Vector2d, Eigen::Matrix2d>> activeRobots;
	...
	if (a1 == -1 || a2 == -1) {
		...
		for(; itr < p.end(); itr++, idx++ )
		{
			if (step < itr->getStateCount()){
				...
				activeRobots.insert({mrmp_pdef_->getInstance()->getRobots()[idx]->getName(), st});
			}
			else{
		...
		if (step < p[a1].getStateCount()) {
			...
			activeRobots.insert({mrmp_pdef_->getInstance()->getRobots()[a1]->getName(), st});
		}
		else {
		...
		if (step < p[a2].getStateCount()) {
			...
			activeRobots.insert({mrmp_pdef_->getInstance()->getRobots()[a2]->getName(), st});
		}
		else {
...
ConflictPtr BeliefPlanValidityChecker::checkForConflicts_(std::unordered_map<std::string, std::pair<Eigen::Vector2d, Eigen::Matrix2d>> states_map, const int step)
{
...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> BeliefPlanValidityChecker::getHalfPlanes_(Polygon combined_poly)
{
...
Polygon BeliefPlanValidityChecker::getMinkowskiSumOfRobots_(Robot* r1, Robot* r2)
{
...
double BeliefPlanValidityChecker::crossProduct_(const Point &a, const Point &b)
{
	/* Performs A \times B */
...
Point BeliefPlanValidityChecker::subtractPoints_(const Point &a, const Point &b)
{
...
Point BeliefPlanValidityChecker::addPoints_(const Point &a, const Point &b)
{
	/* Element-wise subtracts B from A ( i.e. A-B ) */
	const double ax = bg::get<0>(a);
	const double ay = bg::get<1>(a);
