...
{
    OMPL_INFORM("%s: Setting up planning problem for all agents.", "multiAgentSetUp");
    ...
    if (mrmp_solver == "K-CBS" || mrmp_solver == "PBS") {
        if (ll_solver == "RRT") {
            return set_up_KCBS_RRT_instances(mrmp_instance);
        }
        else if (ll_solver == "BSST") {
            return set_up_KCBS_BSST_instances(mrmp_instance);
        }
    }
    else if (mrmp_solver == "MR-RRT") {
        return set_up_MultiRobotRRT_instances(mrmp_instance);
    }
    else {
        OMPL_ERROR("%s: %s is not yet implemented for MRMP.", "multiAgentSetUp", mrmp_solver.c_str());
    }
    return {};
}
    ...
    for (auto itr = robots.begin(); itr != robots.end(); itr++) {
        if ((*itr)->getDynamicsModel() == "2D-Uncertain-Linear-Model") {
            ...
            OMPL_ERROR("%s: Dynamics model named %s is not yet implemented!", 
                "set_up_NonDeterministic_instances", (*itr)->getDynamicsModel().c_str());
            return {};
        }
    }
    OMPL_INFORM("%s: Initialized %lu robots.", "multiAgentSetUp", prob_defs.size());
    return prob_defs;
}
...
std::vector<MotionPlanningProblemPtr> set_up_KCBS_RRT_instances(InstancePtr mrmp_instance)
{
    ...
    for (auto itr = robots.begin(); itr != robots.end(); itr++) {
        if ((*itr)->getDynamicsModel() == "FirstOrderCar") {
            ...
            OMPL_ERROR("%s: Dynamics model named %s is not yet implemented!", 
                "set_up_Deterministic_instances", (*itr)->getDynamicsModel().c_str());
            return {};
        }
    }
    OMPL_INFORM("%s: Initialized %lu robots.", "multiAgentSetUp", prob_defs.size());
...
std::vector<MotionPlanningProblemPtr> set_up_MultiRobotRRT_instances(InstancePtr mrmp_instance)
{
    ...
    //         return probDefs;
    //     }
    OMPL_ERROR("%s: MultiRobotRRT is not yet implemented after code refactor.", "multiAgentSetUp");
    return {};
}
